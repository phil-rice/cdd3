...
class DecisionTreeRenderingSpec extends CddSpec with DecisionTreeFixture with JsonWriterLangauge {
  ...
  it should "render a scenario" in {
    simple.scenario(sNoPassport) shouldBe JsonObject("situation" -> "woman", "url"-> "scenario_0.html", "defined" -> "(ScenarioFixture.scala:13)")
  }
  ...
  it should "render a conclusion node" in {
    simple.node(concGunGunNoPassport) shouldBe JsonObject(
      "conclusionNode" -> JsonObject("scenarios" -> JsonList(List(
        JsonObject("situation" -> "man with gun and passport", "url"-> "scenario_1.html", "defined" -> "(ScenarioFixture.scala:14)"),
        JsonObject("situation" -> "man with gun", "url" -> "scenario_2.html",  "defined" -> "(ScenarioFixture.scala:15)")))),
