...
trait ScenarioFixture extends Matchers {
  ...
  val snomral2 = scenario("man with passport") produces "accept" scenario
  ...
  val saw = scenario("aw") produces "A" scenario
...
trait DecisionTreeFixture extends ScenarioFixture {
  ...
  def dt[P, R](scond: Scenario[P, R], selse: Scenario[P, R]) = DecisionNode(scond.logic, cn(selse), cn(scond))
  ...
  val concNormal1And2 = ConclusionNode(List(snormal, snomral2), snormal.logic)
...
class ConclusionNodeEditorSpec extends CddSpec with DecisionTreeFixture {
  val editor = implicitly[ConclusionNodeEditor[String, String]]

  behavior of "ConclusionNodeEditor"

  def check[P, R](pf: PartialFunction[(ConclusionNode[P, R], Scenario[P, R]), DecisionTree[P, R]])(c: ConclusionNode[P, R], s: Scenario[P, R]) = {
  ...
  it should "add a scenario to a compatible conclusion" in {
    check(editor.compatible)(concNormal, snomral2) shouldBe concNormal1And2
  ...
  it should "make a decision node with scenario on right, if conclusion can be differentiated by the scenario" in {
    implicit val splitter = new ScenarioSplitter(concGun, sNoPassport)
    splitter.canUseScenarioAsCondition shouldBe true
    check(editor.scenarioInRight)(concGun, sNoPassport) shouldBe dGunNoPassword
...
class DecisionTreeSimpleFoldSpec extends CddSpec with DecisionTreeFixture {
  implicit val defaultFunction = SimpleDefaultFunction[String, String](default)
  val folder = DecisionTreeFolder.apply[String, String] _
  ...
  it should "fold a scenario and have a conclusion with just that scenario" in {
    ...
    folder(List(snormal, snomral2)) shouldBe concNormal1And2
  }
  ...
  it should "put the scenario on the right when a different scenario without when is added" in {
    folder(List(sa, sbwb)) shouldBe dt(sbwb, sa)
    folder(List(sa, saw, sax, sbwb)) shouldBe dt(List(sbwb), List(sa, saw, sax))
    folder(List(sa, saw, sax, sbwb, sb)) shouldBe dt(List(sbwb, sb), List(sa, saw, sax))
  }
  ...
  it should "put a scenario without when on the left when a different scenario with when is added" in {
    folder(List(sbwb, sa)) shouldBe dt(sbwb, sa)
